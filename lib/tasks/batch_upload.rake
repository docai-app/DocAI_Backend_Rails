...
namespace :batch_upload do
  ...
  def parent_folder(file_path)
    ...
    Folder.find_or_create_by(name: parent_dir_name)
  end
  ...
  task create_folders: :environment do
    ...
    # 開好 d folder
    Folder.find_or_create_by(name: '建業文件', user:)
    ...
    Dir['/Users/chonwai/Downloads/建業文件/**/*'].each do |f|
      ...
      parent_folder(f).children.find_or_create_by name: folder_name, user:
    end
  end
  ...
  task create_folders_on_root: :environment do
    ...
    Dir['/Users/chonwai/Downloads/建業文件/**/*'].each do |f|
      ...
      puts "#{f} ... "
      folder_name = File.basename(f)
      Folder.find_or_create_by(name: folder_name, user:)
    end
  end
  ...
  task :documents_with_tag, %i[folder_id tag_id] => :environment do |_task, args|
    tag_id = args[:tag_id]
    ...
    Dir['/Users/chonwai/Downloads/建業文件/**/*'].each do |f|
      ...
      # data = File.open(f, "rb")
      # doc = Document.new(folder_id: folder.id, status: "uploaded", upload_local_path: f, name: File.basename(f))
      doc = Document.where(folder_id:).where(status: 'ready').first
