class OpenAiService
  ...
  def self.chatWithDocument(query, content, response_format, language, topic, style)
    ...
    prompt = "You are currently an assistant, and there is a task based on document content generation that requires your assistance. Please help me generate a #{response_format} content about #{topic} using #{language}, and generate it in #{style} style. The output should follow the format \'\'\'json\n {\"content\": \"\"} \'\'\'! The task is as follows: #{query}, with the reference document as follows: #{content}."
