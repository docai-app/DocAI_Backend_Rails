class Conceptmap < ApplicationRecord
  ...
  def create_root_node
    ...
    c = Concept.new(name: name)
    c['meta'] ||= {}
    c['meta']['conceptmap_id'] = self.id
    c.save
  ...
  def lookup_node(path)
    ...
    # 都搵唔到
    # 直接 match node name
    return nodes.where(name: path_array).first
  end

  def dfs_search(node, path)
    # Check if the current node matches the first element of the path
    if node['content'] == path[0]
      return node['meta']['concept_id']
    end
  ...
  def to_markdown(url = nil)
    if url.nil?
      url = "https://examhero.com/android/AIAdmin/admin/concept.html"
    end
  ...
  def generate_markdown(node, url, level=1, prefix="")
    ...
    if url.nil?
      ...
      content = "[#{node['content']}](#{url}?id=#{node['meta']['concept_id']})"
    end
    markdown += "#{prefix}- #{content}\n" #unless level == 1
    node['children'].each do |child|
      markdown += "#{generate_markdown(child, url, level+1, "#{prefix}  ")}"
    end
    markdown
  end
