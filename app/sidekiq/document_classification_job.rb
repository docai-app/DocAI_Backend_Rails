...
class DocumentClassificationJob
    ...
    Apartment::Tenant.switch!(tenant) do
      ...
      if documents_to_classify.count > 5
        ...
        if train_model(tenant, view_name) && predict_first_document(tenant, view_name, documents_to_classify)
          update_documents_as_trained(documents_to_classify)
  ...
  def predict_first_document(tenant, view_name, documents)
    ...
    predict_res = RestClient.get("#{ENV['DOCAI_ALPHA_URL']}/classification/predict",
                                 params: { content: URI.encode_www_form_component(document_content),
                                           model: "#{tenant}_#{view_name}" })
    JSON.parse(predict_res)['status'] == 'Success'
  end

  def update_documents_as_trained(documents)
    ClassificationModelVersion.create_new_version(documents, tenant)
