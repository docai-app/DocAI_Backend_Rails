...
class DocumentClassificationMonitorJob
  ...
  def perform(*_args)
    ...
    Apartment::Tenant.each do |tenant|
      ...
      puts "====== tenant: #{tenant} ======"
      @documents = Document.where(is_classified: true).where.not(content: nil).where.not(content: '').where('LENGTH(content) > ?', 5).where(is_document: true).where(is_classifier_trained: false).where(
        'retry_count < ?', 3
