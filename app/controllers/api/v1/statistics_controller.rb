class Api::V1::StatisticsController < ApiController
  ...
  def count_document_by_date
    @count = Document.includes([:taggings, :labels]).where("is_document = true").by_day(params[:date]).count()
    @confirmed_count = Document.includes([:taggings, :labels]).where("is_document = true").where(status: :confirmed).where("created_at >= ?", params[:date]).count()
    @unconfirmed_count = Document.includes([:taggings, :labels]).where("is_document = true").where.not(status: :confirmed).where("created_at >= ?", params[:date]).count()
    @total_unconfirmed_count = Document.includes([:taggings, :labels]).where("is_document = true").where.not(status: :confirmed).count()
