class Api::V1::FormDatumController < ApiController
  ...
  def show_by_filter_and_form_schema_id
    @form_datum = FormDatum.where(form_schema_id: params[:form_schema_id]).where("data @> ?", params[:filter].to_json).includes([:form_schema, :document]).as_json(include: { document: { except: [:label_list] }, form_schema: {} })
