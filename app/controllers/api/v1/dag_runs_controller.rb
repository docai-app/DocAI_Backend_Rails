class Api::V1::DagRunsController < ApiController

  before_action :set_dag_run, except: [:create, :index]
  ...
  def index
    ...
    json_success(drs.as_json(only: [:id, :dag_name, :dag_status, :created_at, :updated_at]) )
  end
  ...
  def update
    sst = @dag_run.find_status_stack_by_key(params[:task_name])
