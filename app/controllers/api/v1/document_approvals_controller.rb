class Api::V1::DocumentApprovalsController < ApiController
  ...
  def show_normal_documents_by_approval_status
    ...
    @normal_documents = DocumentApproval.where(approval_status: params[:status]).where(form_data_id: nil).where("created_at >= ?", @date).includes([:document]).as_json(include: { document: { except: [:label_list] }, form_data: { except: [:label_list] } })
  ...
  def show_forms_by_approval_status
    @date = Date.tomorrow - params[:days].to_i || Date.today - 3
    @forms = DocumentApproval.where(approval_status: params[:status]).where(form_data_id: FormDatum.where(form_schema_id: params[:form_schema_id])).where("created_at >= ?", @date).includes([:document, :form_data]).as_json(include: { document: { except: [:label_list] }, form_data: { except: [:label_list] } })
