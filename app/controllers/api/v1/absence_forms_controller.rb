...
module Api
  module V1
    class AbsenceFormsController < ApiController
      ...
      def show_by_approval_status
        ...
        @absence_forms = DocumentApproval.where(approval_status: params[:status]).where(form_data_id: FormDatum.where(form_schema_id: FormSchema.where(name: '請假表').first.id)).where(
          'created_at >= ?', @date
        ).includes([:document, :form_data,
                    { document: :taggings }]).as_json(include: %i[document form_data])
        @absence_forms = Kaminari.paginate_array(@absence_forms).page(params[:page])
        render json: { success: true, absence_forms: @absence_forms, meta: pagination_meta(@absence_forms) },
               status: :ok
      end

      # Show absence fomr by approval id
      def show_by_approval_id
        @absence_form = DocumentApproval.find(params[:id]).as_json(include: %i[document form_data])
      ...
      def upload
        ...
        begin
          files.each do |file|
            ...
            @document.save
            recognizeRes = RestClient.post "#{ENV['DOCAI_ALPHA_URL']}/alpha/form/recognize/absence",
                                           { document_url: @document.storage_url }
            recognizeRes = JSON.parse(recognizeRes)
            @form_data = FormDatum.new(data: recognizeRes['absence_form_data'],
                                       form_schema_id: FormSchema.where(name: '請假表').first.id, document_id: @document.id)
            @form_data.save
