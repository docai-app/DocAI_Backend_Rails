class Api::V1::FormProjectionController < ApplicationController
  ...
  def confirm
    ...
    projectionImage = FormProjectionService.preview(@form_schema, params[:data])
    begin
      @document = Document.new(name: filename, created_at: Time.zone.now, updated_at: Time.zone.now, folder_id: target_folder_id)
      @document.storage_url = AzureService.uploadPreviewImage(projectionImage.to_blob, filename, 'image/png') if projectionImage.present?
