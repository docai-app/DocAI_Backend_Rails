...
class CreateOauthEmbedSsoTables < ActiveRecord::Migration[7.0]
  def change
    # Columns may already exist if applied out-of-band or a previous partial run.
    unless column_exists?(:oauth_applications, :allowed_launch_origins)
      add_column :oauth_applications, :allowed_launch_origins, :jsonb, null: false, default: []
    end
    unless column_exists?(:oauth_applications, :sso_launch_enabled)
      add_column :oauth_applications, :sso_launch_enabled, :boolean, null: false, default: false
    end

    unless table_exists?(:oauth_embed_launches)
    ...
    unless table_exists?(:oauth_embed_sessions)
    ...
    unless foreign_key_exists?(:oauth_embed_sessions, :general_users, column: :user_id)
      add_foreign_key :oauth_embed_sessions, :general_users, column: :user_id
    end
    unless foreign_key_exists?(:oauth_embed_sessions, :essay_assignments, column: :assignment_id)
      add_foreign_key :oauth_embed_sessions, :essay_assignments, column: :assignment_id
    end
    unless foreign_key_exists?(:oauth_embed_launches, :essay_assignments, column: :assignment_id)
